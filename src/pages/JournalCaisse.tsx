...
const TYPE_CFG: Record<MouvementType, { label: string; shortLabel: string; badgeClass: string; sign: '+' | '-' | '±' }> = {
  ...
  if (!open) {
    return (
      <Button onClick={() => setOpen(true)} className="bg-cyan-600 hover:bg-cyan-700 text-white">
        <Plus size={14} className="mr-1" /> Alimentation / Prélèvement
      </Button>
    );
  }
    ...
    <Card className="border-cyan-200 bg-cyan-50/50">
      <CardHeader className="pb-3 pt-4 px-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setOpen(false)} className="text-zinc-500 hover:text-zinc-800"><X size={15} /></button>
        </div>
      </CardHeader>
        ...
        <div className="flex flex-wrap gap-3">
          <div className="space-y-1">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => set('sens', 'ALIMENTATION')}
                className={`flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-xs font-semibold transition-colors ${form.sens === 'ALIMENTATION' ? 'border-cyan-400 bg-cyan-100 text-cyan-800' : 'border-zinc-300 bg-white text-zinc-600 hover:border-zinc-400'}`}>
                <ArrowUpCircle size={13} /> Alimentation
              </button>
              <button type="button" onClick={() => set('sens', 'PRELEVEMENT')}
                className={`flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-xs font-semibold transition-colors ${form.sens === 'PRELEVEMENT' ? 'border-rose-400 bg-rose-100 text-rose-800' : 'border-zinc-300 bg-white text-zinc-600 hover:border-zinc-400'}`}>
          ...
          <div className="space-y-1">
            ...
            <select value={form.devise} onChange={(e) => set('devise', e.target.value)}
              ...
              {DEVISES.filter((d) => d !== 'MAD').map((d) => <option key={d} value={d}>{d}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            ...
            <Input type="number" min="0.01" step="0.01" value={form.montant}
              onChange={(e) => set('montant', e.target.value)} placeholder="0.00"
          ...
          <div className="flex-1 space-y-1">
            ...
            <Input value={form.note} onChange={(e) => set('note', e.target.value)}
              placeholder="ex : Apport comptoir, Remise coffre..." className="min-w-[180px]" />
          </div>
...
export function JournalCaisse() {
  ...
  function handleDelete(id: string) {
    ...
    const all = getMouvements().filter((x) => x.id !== id);
    localStorage.setItem('mouvements_caisse', JSON.stringify(all));
    refresh();
  }
  ...
  function exportCSV() {
    const rows = filtered.map((m) => ({
      ID: m.id, Timestamp: m.timestamp, Type: m.type, Devise: m.devise,
      Montant: m.montant, 'Solde avant': m.soldeAvant, 'Solde après': m.soldeApres,
      'Opération réf.': m.operationRef ?? '', Caissier: m.caissier, Note: m.note ?? '', Statut: 'VERROUILLÉ',
    }));
  ...
  return (
    <div className="min-h-screen bg-zinc-50">
      <PageHero title="Journal Caisse — Immuable" subtitle="Audit trail complet · Chaque mouvement est verrouillé 🔒" />
        ...
        <Card>
          <CardContent className="p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button type="button" onClick={() => { setFilterDate(''); setPage(1); }}
                className="h-8 rounded-md border border-zinc-300 bg-white px-2 text-xs text-zinc-600 hover:border-zinc-400">
                Tout
              </button>
              <select value={filterType} onChange={(e) => { setFilterType(e.target.value as MvTypeFull); setPage(1); }}
                className="h-8 rounded-md border border-zinc-300 bg-white px-2 text-xs text-zinc-900">
                <option value="TOUS">Tous types</option>
                {ALL_TYPES.map((t) => <option key={t} value={t}>{TYPE_CFG[t].label}</option>)}
              </select>
              <select value={filterDevise} onChange={(e) => { setFilterDevise(e.target.value); setPage(1); }}
                className="h-8 rounded-md border border-zinc-300 bg-white px-2 text-xs text-zinc-900">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    ...
                    <tbody className="divide-y divide-zinc-100">
                      {pageRows.map((m, i) => (
                        <tr key={m.id} className={`transition-colors hover:bg-zinc-50 ${i % 2 === 0 ? '' : 'bg-zinc-50/50'}`}>
                          ...
                          <td className="px-3 py-2.5"><TypeBadge type={m.type} /></td>
                          <td className="px-3 py-2.5 font-mono font-semibold text-zinc-700">{m.devise}</td>
                          <td className="px-3 py-2.5 text-right tabular-nums">
                            <span className={`font-bold ${signClass(m.montant)}`}>
                              {signPrefix(m.montant)}{fmt(Math.abs(m.montant))}
                            </span>
                          </td>
                          <td className="px-3 py-2.5 text-right tabular-nums text-zinc-500">{fmt(m.soldeAvant)}</td>
                          <td className="px-3 py-2.5 text-right tabular-nums">
                          ...
                          <td className="px-3 py-2.5 text-center">
                           ...
                           {(m.type === 'ALIMENTATION' || m.type === 'PRELEVEMENT' || m.type === 'RELIQUAT') && (
  <button
                        ...
                        .map((p, i) =>
                          p === '...' ? (
                            ...
                            <button key={p} onClick={() => setPage(p as number)}
                              className={`h-7 w-7 rounded-md text-xs font-medium transition-colors ${page === p ? 'bg-blue-600 text-white' : 'text-zinc-600 hover:bg-zinc-100'}`}>
                              {p}
                            </button>
                          )
                        )}
