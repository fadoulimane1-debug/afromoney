...
type CreditStatut = 'En cours' | 'Payé' | 'Retard';
...
function saveCredits(list: Credit[]): void {
...
export function Credits() {
  ...
  function marquerPayé(id: string) {
    ...
    setCredits((prev) =>
      prev.map((x) => (x.id === id ? { ...x, statut: 'Payé' } : x))
    );
    if (c) logAudit(AUDIT_ACTIONS.CREDIT_UPDATE, { id, action: 'marquerPayé' }, c.date);
    showToast('Crédit marqué comme payé', true);
  }
