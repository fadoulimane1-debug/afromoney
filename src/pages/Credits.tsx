...
function StatutBadge({ statut }: { statut: CreditStatut }) {
  ...
  const Icon = cfg.icon;
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold ring-1 ${cfg.badgeClass}`}>
      <Icon size={10} />
      {cfg.label}
    </span>
  );
...
/* ── Modal confirmation crédit existant ── */
function ModalCreditExistant({
  creditExistant,
  montantAAjouter,
  onConfirmer,
  onNouveauCredit,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-xl border border-amber-200 bg-white shadow-2xl">
        <div className="border-b border-amber-100 bg-amber-50 px-5 py-4 rounded-t-xl">
          <div className="flex items-center gap-2">
            <AlertTriangle size={18} className="text-amber-600" />
            <h3 className="font-semibold text-amber-900">Client avec crédit en cours</h3>
          </div>
        </div>
        <div className="px-5 py-4 space-y-3">
          <p className="text-sm text-zinc-700">
            <span className="font-bold text-zinc-900">{creditExistant.nom}</span> a déjà un crédit en cours :
          </p>
          <div className="rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm space-y-1">
            <div className="flex justify-between">
              ...
              <span className="font-bold text-zinc-900">{fmt(creditExistant.contre_val_mad)} MAD</span>
            </div>
            <div className="flex justify-between">
              <span className="text-zinc-500">Restant dû :</span>
              <span className="font-bold text-red-600">{fmt(creditExistant.montantRestant ?? creditExistant.contre_val_mad)} MAD</span>
            </div>
            <div className="flex justify-between border-t border-zinc-200 pt-1 mt-1">
              <span className="text-zinc-500">Montant à ajouter :</span>
              <span className="font-bold text-blue-700">+ {fmt(montantAAjouter)} MAD</span>
            </div>
            <div className="flex justify-between">
              <span className="text-zinc-500">Nouveau restant :</span>
              <span className="font-bold text-red-700">
                {fmt((creditExistant.montantRestant ?? creditExistant.contre_val_mad) + montantAAjouter)} MAD
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-2 px-5 pb-5">
          ...
          >
            <PlusCircle size={14} />
            Ajouter au crédit existant ({fmt((creditExistant.montantRestant ?? creditExistant.contre_val_mad) + montantAAjouter)} MAD)
          </Button>
          <Button
            onClick={onNouveauCredit}
            variant="outline"
            size="sm"
            className="w-full gap-2 border-zinc-300 text-zinc-700"
          >
            <Plus size={14} />
            Créer un nouveau crédit séparé
          </Button>
          <button
            onClick={onAnnuler}
            className="text-xs text-zinc-400 hover:text-zinc-600 text-center py-1"
          >
            Annuler
          </button>
        </div>
...
export function Credits() {
  ...
  /* ── Submit — vérifie si client existe déjà ── */
  function handleSubmit(e: React.FormEvent) {
    ...
    const m = parseMontantStr(form.montant);
    const t = parseMontantStr(form.taux);
    const montantMAD = form.contre_val_mad !== ''
      ? Math.round(parseFloat(form.contre_val_mad) * 100) / 100
      : Math.round(m * t * 100) / 100;

    // Vérifier si ce client a déjà un crédit en cours
    const creditExistant = findCreditEnCours(form.nom);
    if (creditExistant) {
      setModalCreditExistant({
        credit: creditExistant,
        montantMAD,
        note: form.note.trim(),
        formData: { ...form },
      });
    ...
    // Pas de crédit existant → créer normalement
    creerNouveauCredit(form, m, t, montantMAD);
  }

  function creerNouveauCredit(f: FormState, m: number, t: number, montantMAD: number) {
    const newCredit: Credit = {
      ...
      montant: m,
      taux: t,
      contre_val_mad: montantMAD,
      montantRestant: montantMAD,
      note: f.note.trim(),
      statut: f.statut,
      echeance: f.echeance || undefined,
    };
    logAudit(AUDIT_ACTIONS.CREDIT_CREATE, { id: newCredit.id, nom: newCredit.nom, mad: newCredit.contre_val_mad }, newCredit.date);
  ...
  /* ── Confirmer l'ajout au crédit existant ── */
  function handleConfirmerAjout() {
    if (!modalCreditExistant) return;
    const updated = ajouterAuCredit(
      modalCreditExistant.credit.id,
      modalCreditExistant.montantMAD,
      modalCreditExistant.note,
    );
    if (updated) {
      setCredits(loadCredits());
      showToast(
        `+ ${fmt(modalCreditExistant.montantMAD)} MAD ajouté au crédit de ${modalCreditExistant.credit.nom} — nouveau restant : ${fmt(updated.montantRestant)} MAD`,
        true,
      );
    }
    setModalCreditExistant(null);
    setForm(emptyForm());
    setErrors({});
  }

  /* ── Créer quand même un nouveau crédit séparé ── */
  function handleNouveauCreditSepare() {
    if (!modalCreditExistant) return;
    const f = modalCreditExistant.formData;
    const m = parseMontantStr(f.montant);
    const t = parseMontantStr(f.taux);
    creerNouveauCredit(f, m, t, modalCreditExistant.montantMAD);
    setModalCreditExistant(null);
  }
  ...
  function marquerRetard(id: string) {
    ...
    setCredits((prev) => prev.map((x) => (x.id === id ? { ...x, statut: 'Retard' } : x)));
  ...
  return (
    <div>
      ...
      {modalCreditExistant && (
        <ModalCreditExistant
          creditExistant={modalCreditExistant.credit}
          montantAAjouter={modalCreditExistant.montantMAD}
          onConfirmer={handleConfirmerAjout}
          onNouveauCredit={handleNouveauCreditSepare}
          onAnnuler={() => setModalCreditExistant(null)}
        />
      )}
      ...
      <div className="page-content mx-auto max-w-7xl space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-4">
          <Card className={`border-zinc-200 shadow-sm ${totalEnAttente > 0 ? 'border-l-4 border-l-red-400' : 'border-l-4 border-l-zinc-200'}`}>
            <CardHeader className="pb-1">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Total crédits (en attente)</CardTitle>
            </CardHeader>
            <CardContent>
              <p className={`text-2xl font-bold tabular-nums ${totalEnAttente > 0 ? 'text-red-600' : 'text-zinc-400'}`}>
                {fmt(totalEnAttente)} MAD
              </p>
              <p className="mt-1 text-xs text-zinc-500">
                {nbEnCours} en cours
                {nbRetard > 0 && <span className="ml-2 font-semibold text-red-500">· {nbRetard} en retard</span>}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="border-l-4 border-l-amber-400 shadow-sm">
            <CardHeader className="pb-1">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-zinc-500">En cours</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border-l-4 border-l-red-500 shadow-sm">
            <CardHeader className="pb-1">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-zinc-500">En retard</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border-l-4 border-l-emerald-500 shadow-sm">
            <CardHeader className="pb-1">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Payés</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-xl font-bold tabular-nums text-emerald-700">{fmt(totaux.paye)} MAD</p>
              <p className="text-xs text-zinc-400">{credits.filter((c) => c.statut === 'Payé').length} crédit{credits.filter((c) => c.statut === 'Payé').length !== 1 ? 's' : ''}</p>
            </CardContent>
          </Card>
        </div>
        ...
        {agingRows.length > 0 && (
          <Card className="border-zinc-200 shadow-sm">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-semibold text-zinc-900">Ancienneté des créances (hors payé)</CardTitle>
              <p className="text-[10px] text-zinc-500">Basé sur l&apos;échéance saisie ou date + délai paramétré (Paramètres)</p>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {agingTotals.map(({ bucket, total, count }) => (
                  <div key={bucket} className="rounded-lg border border-zinc-200 bg-zinc-50/80 px-3 py-2">
                    <p className="text-[10px] font-semibold uppercase text-zinc-500">{AGING_LABELS[bucket]}</p>
        ...
        <Card className="border-zinc-200 shadow-sm">
          <CardHeader className="border-b border-zinc-100 pb-3">
            ...
            <p className="text-[10px] text-zinc-400">
              Si le client a déjà un crédit en cours, vous pourrez l&apos;ajouter au crédit existant
            </p>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-3 lg:grid-cols-4">
                <Field label="Date *" error={errors.date}>
                  <Input type="date" value={form.date} onChange={(e) => set('date', e.target.value)} max={dayjs().format('YYYY-MM-DD')} className={errors.date ? 'border-red-400 bg-red-50' : ''} />
                </Field>

                <Field label="Nom / Contrepartie *" error={errors.nom}>
                  <Input type="text" placeholder="Ex. Ahmed Benali" value={form.nom} onChange={(e) => set('nom', e.target.value)} className={errors.nom ? 'border-red-400 bg-red-50' : ''} />
                </Field>
                ...
                <Field label="Montant *" error={errors.montant}>
                  <Input type="number" step="0.01" min="0.01" placeholder="Ex. 500" value={form.montant} onChange={(e) => set('montant', e.target.value)} className={errors.montant ? 'border-red-400 bg-red-50' : ''} />
                </Field>

                <Field label="Taux *" error={errors.taux} hint="Auto-rempli depuis les taux du jour">
                  <Input type="number" step="0.0001" min="0.0001" value={form.taux} onChange={(e) => set('taux', e.target.value)} className={errors.taux ? 'border-red-400 bg-red-50' : ''} />
                </Field>

                <Field label="Contre-val MAD" hint="Auto-calculé = Montant × Taux">
                  <Input type="text" readOnly value={form.contre_val_mad !== '' ? fmt(Number(form.contre_val_mad)) : '—'} className="cursor-default border-zinc-200 bg-zinc-50 text-right font-bold tabular-nums text-zinc-900" />
                </Field>

                <Field label="Statut">
                  <NativeSelect value={form.statut} onChange={(v) => set('statut', v as CreditStatut)}>
              ...
              <div className="mt-3">
                <Field label="Note" hint="Optionnel — détail, garantie, échéance…">
                  <textarea rows={2} placeholder="Détail du crédit…" value={form.note} onChange={(e) => set('note', e.target.value)}
                    className="flex w-full resize-none rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 placeholder:text-zinc-400 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </Field>
              </div>
              ...
              {montantPreview !== null && form.nom.trim() && (
                <div className="mt-3 flex items-center gap-3 rounded-lg border border-blue-100 bg-blue-50 px-4 py-2.5 text-sm">
                  ...
                  <span className="text-blue-600">{fmt(parseMontantStr(form.montant || '0') || 0, 0)} {form.devise}</span>
                  <span className="text-zinc-400">×</span>
                  <span className="text-blue-600">{fmtRate(parseMontantStr(form.taux || '0') || 0)}</span>
                  <span className="text-zinc-400">=</span>
                  <span className="font-bold text-blue-900">{fmt(montantPreview)} MAD</span>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <CardTitle className="text-sm font-semibold text-zinc-900">
                  Liste des crédits{' '}
                  <span className="font-normal text-zinc-400">({filtered.length} résultat{filtered.length !== 1 ? 's' : ''})</span>
                </CardTitle>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <select value={filterStatut} onChange={(e) => setFilterStatut(e.target.value as CreditStatut | 'Tous')}
                ...
                <div className="relative">
                  ...
                  <Input placeholder="Rechercher contrepartie…" value={search} onChange={(e) => setSearch(e.target.value)} className="h-8 w-44 pl-7 text-xs" />
                  {search && (
            ...
            {filtered.length > 0 && (
              <div className="mt-3 grid grid-cols-2 gap-2 rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-xs sm:grid-cols-3">
                <div>
                  ...
                  <span className="font-bold text-amber-700">{fmt(filtered.filter((c) => c.statut === 'En cours').reduce((s, c) => s + c.contre_val_mad, 0))} MAD</span>
                </div>
                <div>
                  <span className="text-zinc-500">Retard : </span>
                  <span className="font-bold text-red-600">{fmt(filtered.filter((c) => c.statut === 'Retard').reduce((s, c) => s + c.contre_val_mad, 0))} MAD</span>
                </div>
                <div>
                  <span className="text-zinc-500">Payés : </span>
                  <span className="font-bold text-emerald-700">{fmt(filtered.filter((c) => c.statut === 'Payé').reduce((s, c) => s + c.contre_val_mad, 0))} MAD</span>
                </div>
              </div>
            )}
          </CardHeader>
            ...
            {filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-2 py-14 text-center">
                ...
                <p className="text-sm text-zinc-500">
                  {credits.length === 0 ? 'Aucun crédit enregistré — utilisez le formulaire ci-dessus.' : 'Aucun crédit ne correspond à ces filtres.'}
                </p>
              </div>
            ) : (
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-zinc-200 bg-zinc-50/80">
                      ...
                      <th className="px-3 py-2.5 text-left font-semibold text-zinc-700">Nom / Contrepartie</th>
                      ...
                      <th className="px-3 py-2.5 text-right font-semibold text-zinc-700">Contre-val MAD</th>
                      <th className="px-3 py-2.5 text-right font-semibold text-red-600">Restant dû</th>
                  ...
                  <tbody>
                    {filtered.map((c, i) => {
                      const isEven = i % 2 === 0;
                      return (
                        <tr key={c.id} className={`border-b border-zinc-100 transition-colors hover:bg-zinc-50 ${isEven ? 'bg-white' : 'bg-zinc-50/40'}`}>
                          <td className="whitespace-nowrap px-3 py-2.5 text-zinc-500">{dayjs(c.date).format('DD/MM/YYYY')}</td>
                          <td className="px-3 py-2.5 font-semibold text-zinc-900">
                            ...
                            {(c.ajouts?.length ?? 0) > 0 && (
                              <span className="ml-1.5 rounded-full bg-blue-100 px-1.5 py-0.5 text-[9px] font-semibold text-blue-700">
                                +{c.ajouts!.length} ajout{c.ajouts!.length > 1 ? 's' : ''}
                              </span>
                            )}
                          </td>
                          <td className="px-3 py-2.5 font-bold text-zinc-800">{c.devise}</td>
                          <td className="px-3 py-2.5 text-right tabular-nums text-zinc-700">{fmt(c.montant)}</td>
                          <td className="px-3 py-2.5 text-right tabular-nums text-zinc-500">{fmtRate(c.taux)}</td>
                          <td className="px-3 py-2.5 text-right font-bold tabular-nums text-zinc-900">{fmt(c.contre_val_mad)}</td>
                          ...
                          <td className="whitespace-nowrap px-3 py-2.5 text-zinc-600">{c.echeance ? dayjs(c.echeance).format('DD/MM/YYYY') : '—'}</td>
                          <td className="px-3 py-2.5 text-zinc-600">
                            {c.statut === 'Payé' ? '—' : agingById.has(c.id) ? AGING_LABELS[agingById.get(c.id)!.bucket] : '—'}
                          </td>
                          <td className="max-w-[160px] truncate px-3 py-2.5 text-zinc-500" title={c.note}>{c.note || '—'}</td>
                          <td className="px-3 py-2.5 text-center"><StatutBadge statut={c.statut} /></td>
                          <td className="px-3 py-2.5">
                            {confirmDelete === c.id ? (
                              <div className="flex items-center gap-1.5 text-xs">
                                <button onClick={() => supprimerCredit(c.id)} className="font-semibold text-red-500 hover:text-red-700">Confirmer</button>
                                <span className="text-zinc-400">|</span>
                                <button onClick={() => setConfirmDelete(null)} className="text-zinc-500 hover:text-zinc-700">Annuler</button>
                              </div>
                            ) : (
                              <div className="flex items-center gap-1">
                                {c.statut !== 'Payé' && (
                                  paiementPartielId === c.id ? (
                                    <div className="flex items-center gap-1">
                                      <input type="number" value={montantPartiel} onChange={(e) => setMontantPartiel(e.target.value)}
                                        placeholder={String(c.montantRestant ?? c.contre_val_mad)}
                                        className="h-6 w-24 rounded border border-zinc-300 px-1 text-xs" autoFocus />
                                      <button onClick={() => marquerPayé(c.id, montantPartiel ? parseFloat(montantPartiel) : undefined)}
                                      ...
                                      <button onClick={() => { setPaiementPartielId(null); setMontantPartiel(''); }}
                                ...
                                {c.statut === 'En cours' && (
                                  <button onClick={() => marquerRetard(c.id)} title="Marquer en retard"
                                ...
                                <button onClick={() => setConfirmDelete(c.id)} title="Supprimer"
        ...
        <p className="text-[10px] text-zinc-400">
          ...
          <code className="rounded bg-zinc-100 px-1">credits</code>).
        </p>
      </div>
    </div>
  );
